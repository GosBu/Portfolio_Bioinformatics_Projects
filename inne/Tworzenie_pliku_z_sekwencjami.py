...
@author: magor
"""
# Pobranie 34 sekwencji DNA w formacie FASTA
...
"""
# Wyszukiwanie sekwencji DNA w GenBanku (tak chciałam to zrobić ale wyskakuje błąd: Search Backend failed)

term = "Homo sapiens[Orgn] AND gene[Title]"
handle = Entrez.esearch(db="nucleotide", term=term, retmax=35)
record = Entrez.read(handle)
handle.close()

# Pobieranie szczegółów wyników
filtrowana_lista = [record for record in records if record.seq[:200]]
ids = filtrowana_lista["IdList"]
handle = Entrez.efetch(db="nucleotide", id=ids, rettype="fasta", retmode="text")
records = SeqIO.parse(handle, "fasta")

# Zapisanie wyników do jednego pliku
with open("sekwencje_Homo_sapiens.txt", "w") as output_handle:
