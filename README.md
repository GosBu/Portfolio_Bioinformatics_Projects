...
This portfolio showcases practical projects in bioinformatics, genetic data analysis, and machine learning.  
Each project is based on real biological data — DNA sequences, SNPs, genes, and mutations — and demonstrates skills in:
...
`project_sql/`  
Analysis of patient and genetic test data using SQL queries.  
Focused on identifying tests with multiple patients and analysing the frequency of variants across different test types (SNP Array, NGS, WES, WGS).

**Technologies:** SQL, Relational Databases  
**File:** `pd4657-DATAB-projekt.sql`
...
`project_snakemake/`  
An automated workflow for processing NGS data (FASTQ → QC → Alignment → Variant Calling) using Snakemake and Conda environments.
...
### 3. **Genetic Variant Analysis with Spark (1000 Genomes Project)**

`project_spark_snp_analysis/`  
A project utilising Apache Spark for large-scale genetic data analysis (VCF files from the 1000 Genomes Project).  
The goal was to count SNPs, identify variants in the **CFH** gene, and visualise the variant distribution.
...
`dna_basic_analysis/`  
A Python script for analysing DNA sequences in FASTA format.  
Counts nucleotide frequencies, calculates GC content, and generates bar plots of nucleotide composition (A, C, G, T, GC).
...
A collection of Bash scripts automating NGS data analysis for *E. coli K-12*, from read trimming to variant detection.
...
| ----------------------- | ------------------------------------------------- |
| **Programming**         | Python (Pandas, Matplotlib, Biopython), Bash      |
| **Big Data / Cloud**    | Apache Spark, Google Cloud, AWS                   |
| **Biological Analysis** | Snakemake, FastQC, BWA, Samtools, bcftools, Conda |
| **Databases**           | SQL                                               |
